...
type BasicUser = { id: string; fullName: string };
...
/**
 * canViewAll: whether the current user can see every submission (Super Admin,
 * via view_all_admissions). When true, shows a "Submitted By" filter/column
 * so a Super Admin can see exactly which admin recorded each payment on
 * behalf of a student, and narrow the list down to one admin at a time.
 */
export function PaymentHistoryClient({ canViewAll }: { canViewAll: boolean }) {
  ...
  const load = useCallback(async (statusParam: string, submittedByParam: string) => {
    setLoading(true);
    const params = new URLSearchParams();
    if (statusParam) params.set("status", statusParam);
    if (submittedByParam) params.set("submittedBy", submittedByParam);
    const url = params.toString() ? `/api/admin/payments?${params}` : "/api/admin/payments";
  ...
  useEffect(() => {
    load(status, submittedBy);
  }, [load, status, submittedBy]);

  useEffect(() => {
    if (!canViewAll) return;
    apiFetch("/api/admin/users/basic").then(async (res) => {
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        {canViewAll && (
          <select
            ...
            title="Filter by which admin recorded the payment"
          >
      ...
      <TableWrap>
        ...
        <tbody>
          {loading && <TableEmpty colSpan={11}>Loading…</TableEmpty>}
          {!loading && visible.length === 0 && (
            <TableEmpty colSpan={11} icon={<History className="size-6" />}>
          ...
          {!loading &&
            visible.map((s) => (
              <Tr key={s.id}>
                ...
                {/* Which admin recorded this payment on behalf of the student. */}
                <Td className="font-medium text-[#374151]">{s.submittedBy.fullName}</Td>
