...
/**
 * canViewAll: Super Admin (view_all_admissions) — sees every admission with a
 * balance due, plus a "Filled By" filter to narrow down to one admin's
 * admissions. A plain Admin only ever sees their own; the API enforces this.
 */
export function DuePaymentsClient({ canViewAll }: { canViewAll: boolean }) {
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        {canViewAll && (
          <select
            ...
            title="Filter by which admin filled this admission"
          >
      ...
      <TableWrap>
        ...
        <tbody>
          {loading && <TableEmpty colSpan={canViewAll ? 10 : 9}>Loading…</TableEmpty>}
          {!loading && visible.length === 0 && (
            <TableEmpty colSpan={canViewAll ? 10 : 9} icon={<CalendarClock className="size-6" />}>
            ...
            visible.map((r) => {
              ...
              return (
                <Tr key={r.id} className={overdue ? "bg-amber-50/60" : undefined}>
                  ...
                  {canViewAll && (
                    <Td className="text-[#6B7280]">{r.createdBy?.fullName || "—"}</Td>
                  )}
                  <Td>
                    <Link
                      href={`/admin/admissions/${r.id}`}
