...
import { Plus, Pencil, Trash2, Inbox, AlertCircle, X, Eye } from "lucide-react";
...
export function CoursesClient({ canManage }: { canManage: boolean }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      universityFee: form.universityFee || null,
      totalAdminFee: form.totalAdminFee || null,
  ...
  function handleEdit(course: Course) {
    ...
    setForm({
      ...
      totalAdminFee: course.totalAdminFee || "",
  ...
  return (
    <div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <Field label="Total University Fee" hint="Leave blank if configurable">
                <input
                  className={controlClass}
                  placeholder="e.g. 20000"
                  value={form.universityFee}
                  onChange={(e) => setForm({ ...form, universityFee: e.target.value })}
                />
              </Field>
              <Field label="Total Admin Fee" hint="Chahar Institute's admission handling charge">
                <input
                  className={controlClass}
                  placeholder="e.g. 2000"
                  value={form.totalAdminFee}
                  onChange={(e) => setForm({ ...form, totalAdminFee: e.target.value })}
                />
              </Field>
            </div>
      ...
      <TableWrap>
        <thead>
          <tr>
            ...
            <Th className="hidden sm:table-cell">Total University Fee</Th>
            <Th className="hidden sm:table-cell">Total Admin Fee</Th>
        ...
        <tbody>
          ...
          {!loading && courses.length === 0 && (
            <TableEmpty colSpan={10} icon={<Inbox className="size-6" />}>
              ...
              <Tr key={course.id}>
                ...
                <Td className="hidden whitespace-nowrap text-[#6B7280] sm:table-cell">
                  {course.universityFee
                    ? `₹${Number(course.universityFee).toLocaleString("en-IN")}`
                    : "Configurable"}
                </Td>
                <Td className="hidden whitespace-nowrap text-[#6B7280] sm:table-cell">
                  {course.totalAdminFee
                    ? `₹${Number(course.totalAdminFee).toLocaleString("en-IN")}`
                    : "—"}
                </Td>
                <Td>
                  {canManage ? (
      ...
      {/* Read-only detail view — available to everyone, including Admin. */}
      <Dialog open={viewCourse !== null} onOpenChange={(open) => !open && setViewCourse(null)}>
        <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
          {viewCourse && (
            <>
              <DialogHeader>
                <DialogTitle>{viewCourse.courseName}</DialogTitle>
              </DialogHeader>

              <div className="flex flex-col gap-4 text-sm">
                <div className="grid grid-cols-2 gap-3">
                  <DetailField label="Category" value={viewCourse.admissionCategoryName} />
                  <DetailField label="Type" value={viewCourse.courseTypeName} />
                  <DetailField label="Faculty" value={viewCourse.facultyName} />
                  <DetailField label="Eligibility" value={viewCourse.eligibility} />
                  <DetailField label="Duration" value={viewCourse.duration} />
                  <DetailField label="Semesters" value={viewCourse.semesters?.toString() ?? null} />
                  <DetailField
                    label="Total University Fee"
                    value={
                      viewCourse.universityFee
                        ? `₹${Number(viewCourse.universityFee).toLocaleString("en-IN")}`
                        : "Configurable"
                    }
                  />
                  <DetailField
                    ...
                    value={
                      viewCourse.totalAdminFee
                        ? `₹${Number(viewCourse.totalAdminFee).toLocaleString("en-IN")}`
                        : null
                    }
                  />
                </div>
                ...
                {viewCourse.description && (
                  <div>
                    ...
                    <p className="text-[#374151]">{viewCourse.description}</p>
                  </div>
                )}
                ...
                {viewCourse.overview && (
                  <div>
                    ...
                    <p className="text-[#374151]">{viewCourse.overview}</p>
                  </div>
                )}

                {viewCourse.requiredDocuments.length > 0 && (
                  <div>
                    <p className="mb-1.5 text-xs font-semibold uppercase tracking-[0.08em] text-[#9CA3AF]">
                      Required Documents
                    </p>
                    <ul className="flex flex-col gap-1">
                      {viewCourse.requiredDocuments.map((doc) => (
                        <li key={doc} className="text-[#374151]">
                          • {doc}
                        </li>
                      ))}
                    </ul>
                ...
                {viewCourse.careerOpportunities.length > 0 && (
                  <div>
                    <p className="mb-1.5 text-xs font-semibold uppercase tracking-[0.08em] text-[#9CA3AF]">
                      Career Opportunities
                    </p>
                    <div className="flex flex-wrap gap-1.5">
                      {viewCourse.careerOpportunities.map((career) => (
                        <span
                          key={career}
                          className="rounded-full bg-[#FDFBF7] px-2.5 py-1 text-xs text-[#374151]"
                        >
                          {career}
                        </span>
                      ))}
                ...
                {viewCourse.faqs.length > 0 && (
                  <div>
                    <p className="mb-1.5 text-xs font-semibold uppercase tracking-[0.08em] text-[#9CA3AF]">
                      FAQs
                    </p>
                    <ul className="flex flex-col gap-2">
                      {viewCourse.faqs.map((faq) => (
                        <li key={faq.question}>
...
function DetailField({ label, value }: { label: string; value: string | null | undefined }) {
  return (
    <div>
      <p className="text-xs font-semibold uppercase tracking-[0.08em] text-[#9CA3AF]">{label}</p>
